...
skip_worker_init = get(ENV, "NOFASTSCAN_SKIP_WORKERS", "0") == "1"
...
if skip_worker_init
    println("Skipping worker initialization due to NOFASTSCAN_SKIP_WORKERS=1")
elseif !is_slurm_env || nprocs() > 1
...
function TDGLThermalParams(;
    ...
    Je=0.0, He=0.0, bc_x=:periodic, bc_y=:neumann, bc_y_coverage=1.0, loop=false,
    field_side=:both,
    ...
    return TDGLThermalParams(Nx, Ny, hx, hy, dt, kappa, s1, s2, s3, u, gamma,
                             ...
                             Je, He, bc_x, bc_y, bc_y_coverage, loop, field_side,
...
function with_current(p::TDGLThermalParams, Je::Float64)
    return TDGLThermalParams(p.Nx, p.Ny, p.hx, p.hy, p.dt,
                             ...
                             Je, p.He, p.bc_x, p.bc_y, p.bc_y_coverage, p.loop, p.field_side,
...
function apply_boundary_conditions!(state::TDGLThermalState, p::TDGLThermalParams)
    ...
    for j in 1:p.Ny
        if p.bc_x == :neumann
            state.Ay[1, j] = state.Ay[2, j]
            state.Ay[p.Nx+1, j] = state.Ay[p.Nx, j]
...
function build_sweep_config(cfg)
    ...
    input_folder = if isnothing(raw_input_folder)
        nothing
    else
...
function discover_input_folder_points(input_folder::String)
    ...
    isempty(parsed) && error("No continuation H5 files found in $input_folder")
...
@everywhere function select_animation_time_trace(V_t_up::Dict{Float64, Vector{Tuple{Float64,Float64}}},
                                                 V_t_down::Dict{Float64, Vector{Tuple{Float64,Float64}}},
                                                 direction::String, Je::Float64)
...
@everywhere function generate_single_Je_animation_remote(frames_dir::String, Je::Float64, data,
    ...
    for t_idx in sampled_indices
        ...
        p3 = plot(xlabel="Je", ylabel="V", title="I-V Characteristic",
                  legend=:topleft, xlims=(0, Je_max * 1.1), ylims=(0, max(V_max_all, 1e-6)))
        if !isempty(Je_up)
        ...
        p4 = plot(xlabel="t", ylabel="V", title=@sprintf("V vs t (Je=%.3f)", Je),
                  legend=:topright)
        V_t_array = select_animation_time_trace(V_t_up, V_t_down, direction, Je)
        if !isempty(V_t_array)
            times_array = [t for (t, _) in V_t_array]
            V_array = [v for (_, v) in V_t_array]
            plot!(p4, times_array, V_array, label=@sprintf("Je=%.3f", Je), lw=1.5)
        end
